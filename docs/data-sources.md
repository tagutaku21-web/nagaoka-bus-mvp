...
## 2026-09-11 上越市街地路線の試験補完

上越市のGTFSは市営バス中心で、市街地の主要な一般路線が不足するため、頸城自動車公式ページの「路線図と時刻表」を出典に、まず `2.上越大通り線` だけを試験的に補完した。

- 頸城自動車公式ページ: https://www.marukei-g.com/pages/20/
- 対象PDF:
  - `2.上越大通り線` https://www.marukei-g.com/files/libs/7361/202603161517485.pdf
- 補完方法: PDF全体の再掲載ではなく、アプリ検索用に停留所名・停留所順・発着時刻を構造化する。PDF上の `↓` と `‐` は、その便の検索停車対象から外す。
- 対象範囲: `福橋東・直江津港 -> 直江津駅前 -> 西城病院前 -> 中央病院` と、逆向きの `上越妙高駅前 / 中央病院 -> 西城病院前 -> 直江津駅前 -> 直江津港・福橋東`。
- 今回の補完結果: 停留所 60 / 便 21（平日往路5・復路10、土休日往路2・復路4）。
- 停留所位置: OpenStreetMap の `highway=bus_stop` / `public_transport=platform` から、停留所名が一致したものだけを採用する。上越大通り線2番の60停留所中、現時点で位置確認できたのは11停留所。位置確認できない停留所は時刻検索には使うが、地図ピン・ルート線には出さない。
- OSM確認範囲: 上越市街地周辺（概ね緯度 37.04-37.22 / 経度 138.20-138.34）。OSMデータは ODbL。
- 注意: 補完部分は公式GTFSではない。実際の乗り場位置確認には公式時刻表・現地案内が必要。
